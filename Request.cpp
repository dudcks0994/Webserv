...
void Request::ParseRequest(int fd, std::string& buff) 
{
    ...
    m_origin_fd = fd;
    start = 0;
    m_totalBytes += buff.length();
    if (m_stage < 2 && m_totalBytes > 200000000)
    {
        m_status = 431;
        m_reqClose = 1;
        m_complete = 1;
...
void Request::PrintRequest() 
{
    ...
    if (m_body.length() > 100)
        std::cout << m_body.substr(0, 100) << "..." << std::endl;
    else
...
void Request::checkEssential() 
{
    ...
    if (m_headers.find("transfer-encoding") != m_headers.end() && m_headers.find("transfer-encoding")->second == "chunked")
        m_chunked = 1;
    if (m_headers.find("content-length") != m_headers.end() || m_chunked == 1)
    {
        if (m_headers.find("content-length") != m_headers.end() && m_chunked == 1)
        {
            m_status = 400;
            std::cout << "Content-Length and Transfer-Encoding should not be in the same request\n";
            m_reqClose = 1;
            m_end = 1;
        }
    ...
    if (m_headers.find("host") == m_headers.end() || m_headers.find("host")->second == "" || m_headers.count("host") > 1)
    {
        m_status = 400;
    ...
    if (m_path.find("/..") != std::string::npos || m_path.find("/../") != std::string::npos || m_path.find("../") != std::string::npos)
    {
        m_status = 400;
        std::cout << "URI is invalid\n";
        m_reqClose = 1;
        m_end = 1;
    }
    ...
    getsockname(m_origin_fd, (struct sockaddr*)&addr, (socklen_t*)&addr_len);
    int origin_port = ntohs(addr.sin_port);
    m_host_port = origin_port;
}
...
long long Request::convertHex(const std::string& s) 
{
    long long num = 0;
    if (isHex(s))
        return (-1);
...
void Request::SetComplete(int status) 
{
    m_status = status;
    m_complete = 1;
    m_end = 1;
...
long long Request::GetTotalBytes() 
{
    return (m_totalBytes);
}

int Request::GetStage() 
{
    return (m_stage);
}
